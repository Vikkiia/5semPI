...
int main() {
    ...
    try {
        ...
        while (true) {
            bool request = GetRequestFromClient(serverSocket, serverName.c_str(), &client, &len);
            if (request) {
                ...
                cout << "Request from: " << ip << ":" << port << endl;
...
bool checkOtherServers(const char* name, short port, std::vector<ServerInfo>& servers) //
{
    ...
    sendto(sock, name, strlen(name), 0, (sockaddr*)&broadcastAddr, sizeof(broadcastAddr));
