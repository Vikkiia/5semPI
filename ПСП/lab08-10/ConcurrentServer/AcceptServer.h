...
bool AcceptCycle(int squirt, SOCKET* s) {
	...
	if ((c.s = accept(*s, (sockaddr*)&c.prms, &c.lprms)) == INVALID_SOCKET) {
		...
		// Регистрация подключения клиента
		SYSTEMTIME stt;
		GetLocalTime(&stt);
		char dst[16] = "";
		inet_ntop(AF_INET, &(c.prms.sin_addr), dst, sizeof dst);
		printf("%d.%d.%d %d:%02d Client connected from %s\n", stt.wDay, stt.wMonth, stt.wYear, stt.wHour, stt.wMinute, dst);
		LeaveCriticalSection(&scListContact);
		SetEvent(Event);
	}
	//}
