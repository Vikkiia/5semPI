...
namespace ht
{
	...
	{
		this->capacity = 0;
		this->secSnapshotInterval = 0;// переодичность сохранения в сек. 
		this->maxKeyLength = 0;
		...
		const wchar_t* fileName)		
	{
		...
		int sizeMap = sizeof(HtHandle) + getSizeElement(maxKeyLength, maxPayloadLength) * capacity;//размер памяти файла мапинга
		...
		//возвращает указатель на память, где лежит содержимое файла.	
	...
	{
		WaitForSingleObject(htHandle->mutex, INFINITE);
		if (!FlushViewOfFile(htHandle->addr, NULL)) {//сбрасывает изменения из памяти в файл на диске
		...
		const Element* element)			
	{
		...
		do
		{
			if (index >= htHandle->capacity)//не выходим ща предел таьоицы
			{
				index = -1;
				break;
			}
